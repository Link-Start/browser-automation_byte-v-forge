...
func (r *Runtime) startWorker(ctx context.Context, session *core.Session, proxyURL string) (*workerProcess, error) {
	optionsMap, err := workerOptions(r.cfg, session, proxyURL)
	if err != nil {
	...
	cmd := exec.Command(r.cfg.PythonPath, "-u", "-c", r.script)
	cmd.SysProcAttr = &syscall.SysProcAttr{Setpgid: true}
	cmd.Env = append(os.Environ(), r.cfg.ExtraEnv...)
	cmd.Env = append(cmd.Env, "CLOAK_BROWSER_WORKER_OPTIONS_JSON="+options)
	...
	if err := json.Unmarshal([]byte(line), &ready); err != nil || ready.Type != "ready" {
		_ = worker.stop(r.cfg.ShutdownTimeout)
		if err != nil {
			return nil, core.NewError(core.CodeBrowserUnavailable, "cloakbrowser worker readiness response is invalid: "+err.Error(), true)
		}
		return nil, core.NewError(core.CodeBrowserUnavailable, "cloakbrowser worker readiness response is invalid", true)
	}
	return worker, nil
}
...
func (p *workerProcess) readLine(ctx context.Context, timeout time.Duration) (string, error) {
	...
	case line, ok := <-p.lines:
		if !ok {
			return "", core.NewError(core.CodeBrowserUnavailable, p.failureMessage("cloakbrowser worker stdout closed", nil), true)
		}
		return line, nil
	case err := <-p.done:
		return "", core.NewError(core.CodeBrowserUnavailable, p.failureMessage("cloakbrowser worker exited", err), true)
	case <-timer.C:
		return "", core.NewError(core.CodeTimeout, p.failureMessage("cloakbrowser worker timed out", nil), true)
	case <-ctx.Done():
