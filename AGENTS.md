...
- 公开浏览器自动化契约来自 `common-lib/proto/byte/v/forge/contracts/browserautomation/v1/`；runtime 私有配置、provider raw metadata 和内部执行状态留在本仓内部 proto。
...
- `gen/` 只承载本仓内部 proto 生成物；公开 Go 契约生成物来自 `common-lib/gen/go/byte/v/forge/contracts/browserautomation/v1/`。
