  ...
  return (
    <Card className="card result-card">
      <div className="card-title">
        <Box><Text as="p" size="5" weight="bold">结果</Text></Box>
        <Badge color={task ? statusThemeColor(task.status) : 'gray'} variant="soft">{task ? statusLabel(task.status) : '空闲'}</Badge>
      </div>
      {task ? (
        <div className="result-body">
          <div className="result-line"><span>任务</span><strong title={task.task_id}>{task.task_id}</strong></div>
          ...
          {taskHistoryPath ? <Button asChild size="2" variant="soft"><Link to={taskHistoryPath}><Rows3 size={14} />记录</Link></Button> : null}
        </div>
      ) : (
        <EmptyState
          action={taskHistoryPath ? <Button asChild size="2" variant="soft"><Link to={taskHistoryPath}><Rows3 size={14} />记录</Link></Button> : undefined}
          description="无结果"
...
function resultPageLabel(task: BrowserTask) {
  const firstResult = task.results?.[0];
  if (firstResult?.title) return safeMessage(firstResult.title);
